...
const BASE_AMOUNT_IN = BigInt(0.0001 * LAMPORTS_PER_SOL);
const QUOTE_AMOUNT_OUT = BigInt(0);
const TRANSFER_MIN_AMOUNT = (0.0001 * LAMPORTS_PER_SOL).toString();
        ...
        stream.write(
            {
                ...
                transactions: {
                    createPool: {
                        ...
                        accountExclude: [],
                        accountRequired: [targets[0].toBase58(), pumpswap.address],
                    },
                    transfer: {
                        vote: false,
                        failed: false,
...
eventEmitter.on("mint", async (pool: PublicKey, baseMint: PublicKey, quoteMint: PublicKey, buyerIndex: number, recentBlockhash: string) => {
    ...
    console.log(baseMint.toBase58(), "buy transaction sent", bs58.encode(tx.signatures[0]));
...
eventEmitter.on("exit", async (pool: PublicKey, baseMint: PublicKey, quoteMint: PublicKey, buyerIndex: number) => {
    ...
    //TODO implement a better exit strategy
    ...
    console.log(baseMint.toBase58(), "Sell transaction sent", bs58.encode(sellTx.signatures[0]));
});
...
eventEmitter.on(
    ...
    async (
        ...
        if (decodedIx.lamports < TRANSFER_MIN_AMOUNT) {
            return;
        }
...
eventEmitter.on("transaction", async (grpcResponse: GRPCResponse) => {
    console.log("Transaction received:", grpcResponse.transaction.transaction.message);
    const accountKeys = grpcResponse.transaction.transaction.message.accountKeys.map((key) => new PublicKey(key));
    const transferIxs = grpcResponse.transaction.transaction.message.instructions.filter((ix) => {
        if (!accountKeys[ix.programIdIndex].equals(SystemProgram.programId)) return false;
        const data = bs58.decode(ix.data);
        if (data.length < 4) return false;
        const instructionDiscriminator = data.readUInt32LE(0);
        return instructionDiscriminator === 2;
    });
    transferIxs.map((ix) => {
        eventEmitter.emit("transfer", ix, accountKeys);
    });

    if (!buyers.find((buyer) => buyer.publicKey.toBase58() === grpcResponse.transaction.transaction.message.accountKeys[0])) return;
